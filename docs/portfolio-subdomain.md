# Portfolio subdomain: `ratelimiter.kaustuvbasak.com`
...
| Type  | Name            | Target                          |
|-------|-----------------|---------------------------------|
| CNAME | `ratelimiter`  | `rate-limiter-xxxx.onrender.com` |
...
|-------|-----------------|---------------------------------|-------|
| CNAME | `ratelimiter`  | `<your-service>.onrender.com`   | DNS only recommended |
...
```bash
dig +short ratelimiter.kaustuvbasak.com CNAME
curl -s https://ratelimiter.kaustuvbasak.com/health
```
...
**Link:** https://ratelimiter.kaustuvbasak.com  
...
<article class="project-card">
  ...
  <a
    href="https://ratelimiter.kaustuvbasak.com"
    target="_blank"
    rel="noopener noreferrer"
  >
    Visit ratelimiter.kaustuvbasak.com (opens in new tab)
  </a>
</article>
```
...
```text
Live demos: dns.kaustuvbasak.com · hvac.kaustuvbasak.com · ratelimiter.kaustuvbasak.com
```
...
```xml
<url>
  <loc>https://ratelimiter.kaustuvbasak.com/</loc>
...
- [ ] Add CNAME `ratelimiter` → your `*.onrender.com` host
- [ ] TLS shows as active on Render
- [ ] `curl https://ratelimiter.kaustuvbasak.com/health` succeeds
...
| Issue | Fix |
|-------|-----|
| DNS not verifying | Confirm CNAME name is `ratelimiter`, not `ratelimiter.kaustuvbasak.com` as the name field |
